...
func TestSkipRefreshOnGapCpls(t *testing.T) {
	...
	qFuncWithIgnore := func(rt *kb.RoutingTable, ignoreCpl uint) func(c context.Context, key string) error {
		return func(c context.Context, key string) error {
			...
			require.NoError(t, err)
			b, err := rt.TryAddPeer(p, nil, true, false)
			require.NoError(t, err)
			require.True(t, b)
			return nil
		}
	...
	rt, err := kb.NewRoutingTable(2, kb.ConvertPeerID(local), nil, time.Hour, pstore.NewMetrics(), 100*time.Hour, nil)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	b, _ := rt.TryAddPeer(p, nil, true, false)
	require.True(t, b)
